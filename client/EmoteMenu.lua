...
function AddEmoteMenu(menu)
    ...
    if Config.SharedEmotesEnabled then
        ...
        shareddancemenu.OnIndexChange = function(menu, newindex)
            -- play the emote on ClonedPed
            PlayCloneEmote(DanceTable[newindex], "shared", 'Dances')
        end
    end
...
function CreateClone()
    ...
    local _, _, upT, _ = GetEntityMatrix(PlayerPedId())
    upT = upT - vector3(0.0, 0.0, 0.5)
    CreateThread(function()
        while DoesEntityExist(clone) do
            local forward, right, up, pos = GetEntityMatrix(PlayerPedId())
            SetEntityMatrix(clone, forward, right, upT, pos)
    ...
    CreateThread(function()
        while DoesEntityExist(clone) do
            Citizen.Wait(0)
            scoords, ncoords = GetWorldCoordFromScreenCoord(0.35, 0.2)
            targetCoords = scoords + ncoords * 3.0
            targetHeading = 180 - math.atan(ncoords.x, ncoords.y) * 180.0 / math.pi
        end
    end)
...
function OnCloneEmotePlay(EmoteName, name, textureVariation)
    ...
    if ChosenDict == "MaleScenario" or "Scenario" then
        ...
        elseif ChosenDict == "ScenarioObject" then
            ...
            ClearPedTasks(clonedPed)
            TaskStartScenarioAtPosition(clonedPed, ChosenAnimation, BehindPlayer['x'], BehindPlayer['y'],
                BehindPlayer['z'], GetEntityHeading(clonedPed), 0, true, false)
            DebugPrint("Playing scenario = (" .. ChosenAnimation .. ")")
            IsInAnimation = true
            RunAnimationThread()
            return
    ...
    TaskPlayAnim(clonedPed, ChosenDict, ChosenAnimation, 5.0, 5.0, AnimationDuration, MovementType, 0, false, false,
        false)
...
function OnCloneSharedEmotePlay(EmoteName, name, textureVariation)
    ...
    local emote2 = tostring(EmoteName[4])
    ...
    if AnimationOptions then
        if RP.Shared[emote2].AnimationOptions then
            if RP.Shared[emote2].AnimationOptions.Attachto then
                emote = tostring(EmoteName[4])
                emote2 = tostring(name)
                AnimationOptions = RP.Shared[emote] and RP.Shared[emote].AnimationOptions
            end
        end
    ...
    if RP.Shared[emote] ~= nil then
        OnCloneEmotePlay(RP.Shared[emote], emote)
    end
    if RP.Shared[emote2] ~= nil then
        OnClone2EmotePlay(RP.Shared[emote2], emote2)
    end
...
function AddPropToClone(prop1, bone, off1, off2, off3, rot1, rot2, rot3, textureVariation)
    local Player = clonedPed
...
function PlayCloneEmote(args, hard, textureVariation)
    ...
    elseif etype == "shared" then
        if RP.Shared[name] ~= nil then
            OnCloneSharedEmotePlay(RP.Shared[name], name)
        end
    end
end
